...
if __name__ == "__main__":  # When ran as script for debugging
    import sys, os

    HARDWARE_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', 'hardware'))
    sys.path.append(HARDWARE_DIR)
    import greateyesSDK as ge

else:  # When called from daq_viewer in normal use
...
class DAQ_2DViewer_GreateyesCCD(DAQ_Viewer_base):
    ...
    def ini_detector(self, controller=None):
        ...
        try:
            ...
            self.emit_status(
                ThreadCommand("show_splash", ["Taking one image"])
            )
                ...
                [
                    DataFromPlugins(
                        ...
                        data=[self.controller.PerformMeasurement_Blocking_DynBitDepth(
                            correctBias=self.settings.child("acquisition_settings",
                                                            "do_correct_bias").value())],
                        dim="Data2D",
                        labels=["dat0"],
                        x_axis=self.y_axis,
                        y_axis=self.x_axis,
            ...
            self.settings.child("acquisition_settings", "timing_settings", "last_meas_time").setValue(
    ...
    def ini_greateyes_camera(self):
        ...
        self.settings.child("camera_settings", "dll_version").setValue(self.controller.GetDLLVersion())
        ...
        self.settings.child("acquisition_settings", "timing_settings", "exposure_time").setLimits(
        ...
        if self.controller.SupportedSensorFeature(1):  # Checks if Horizontal Binning is supported
            ...
            self.settings.child("acquisition_settings", "image_settings", "bin_x").hide()

        self.settings.child("acquisition_settings", "image_settings", "bin_y").setLimits(
        ...
        for index, speed in enumerate(speeds):
            ...
            if self.controller.SetReadOutSpeed(
                    eval("self.controller.readoutSpeed_" + str(speed) + "_" + speedUnits[index])):
                availableSpeeds.append(str(speed) + " " + speedUnits[index])
        self.settings.child("acquisition_settings", "timing_settings", "readout_speed").setLimits(availableSpeeds)
        # by default set camera to the fastest readout speed
        self.settings.child("acquisition_settings", "timing_settings", "readout_speed").setValue(availableSpeeds[-1])

        # Set Functions
        # =================================================
        # Set bit depth
        self.controller.SetBitDepth(
            self.settings.child("acquisition_settings", "image_settings", "bit_depth").value())
        ...
        self.controller.SetExposure(
            self.settings.child("acquisition_settings", "timing_settings", "exposure_time").value())
        ...
        callback.exposure = self.settings.child("acquisition_settings", "timing_settings", "exposure_time").value()
        callback.timeout = self.settings.child("acquisition_settings", "timing_settings", "timeout").value()
    ...
    def commit_settings(self, param):
        ...
        if param.name() == "connection_type":
            ...
            if not self.controller.SetReadOutSpeed(eval("self.controller.readoutSpeed_" + speed + "_" + unit)):
                self.emit_status(ThreadCommand('Update_Status', ['Could not update readout speed', 'log']))
    ...
    def update_image(self):
        ...
        self.controller.ActivateCropMode(
            self.settings.child('acquisition_settings', 'image_settings', 'do_crop').value())
    ...
    def close(self):
        ...
        if self.controller.DisconnectCamera():
            if self.settings.child("camera_settings", "connection_type").value() == "Ethernet":
    ...
    def grab_data(self, Naverage=1, **kwargs):
        ...
        """
        try:
            self.settings.child("camera_settings", "camera_status").setValue("Acquiring...")

            self.prepare_data()

            # Start acquisition
            self.controller.StartMeasurement_DynBitDepth(correctBias=self.settings.child("acquisition_settings",
                                                                                         "do_correct_bias").value())
    ...
    def prepare_data(self):
        ...
        if data_shape != self.data_shape:
            ...
            self.data_grabed_signal_temp.emit([DataFromPlugins(name='Camera ',
                                                               data=[np.squeeze(
                                                                   np.zeros((height, width)).astype(np.float))],
                                                               dim=self.data_shape,
                                                               labels='Camera')])
    ...
    def emit_data(self):
        ...
        try:
            if self.callback_thread.callback.timedout:
                self.emit_status(ThreadCommand('Update_Status', ["Measurement timed out", 'log']))
                ...
                self.data_grabed_signal.emit([DataFromPlugins(name='Camera',
                                                              ...
                                                              dim=self.data_shape,
                                                              labels="Camera")])
                self.settings.child("camera_settings", "camera_status").setValue("Data received")

                QtWidgets.QApplication.processEvents()  # here to be sure the timeevents are executed even if in continuous grab mode

                if self.settings.child("acquisition_settings", "timing_settings", "check_meas_time"):
                    self.settings.child("acquisition_settings", "timing_settings", "last_meas_time").setValue(
                        '{:.1f}'.format(self.controller.GetLastMeasTimeNeeded() * 1000))
        except Exception as e:
            self.emit_status(ThreadCommand('Update_Status', [str(e), 'log']))
...
class GreateyesCallback(QtCore.QObject):
    ...
    def wait_for_acquisition(self):
        ...
        t_crit = self.exposure + self.timeout  # time after which we cancel the measurement
